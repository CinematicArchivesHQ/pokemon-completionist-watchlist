...
const icon = (n: string) => `/assets/icons/${n}`;
...
export default function Home() {
  ...
  return <main className="app-shell">
    <header className="topbar">
      <button className="brand" onClick={() => setTab("journey")}><img src="/assets/core/06-journal-cover.png" alt=""/><span><b>Pokémon</b><em>Trainer Journal</em></span></button>
      ...
      <button className="avatar"><img src="/assets/core/07-trainer-avatar-fox-card.png"/><span>{watched}<small>logged</small></span></button>
    </header>

    <section className="hero paper">
      <img className="hero-art" src="/assets/core/01-hero-landscape.png" alt="A painted Pokémon journey landscape"/>
      ...
      <img className="seal" src="/assets/decor/04-vermilion-journey-seal.png" alt=""/>
    </section>

    {tab === "journey" && <>
      <section className="stat-grid">
        <article className="paper"><img src="/assets/core/08-open-watch-log.png"/><div><small>Stories logged</small><strong>{watched.toLocaleString()}</strong><span>of {CATALOG.length.toLocaleString()}</span></div></article>
        <article className="paper"><img src="/assets/core/10-episode-film-projector.png"/><div><small>Episodes</small><strong>{Object.keys(progress).filter(id=>id.startsWith("ep-")&&progress[id].watched).length}</strong><span>television adventures</span></div></article>
        <article className="paper"><img src="/assets/core/09-trainer-backpack.png"/><div><small>Movies & specials</small><strong>{Object.keys(progress).filter(id=>!id.startsWith("ep-")&&progress[id].watched).length}</strong><span>rare discoveries</span></div></article>
        <article className="paper"><img src="/assets/core/04-achievement-patch.png"/><div><small>Completion</small><strong>{pct}%</strong><span>{pct>=25?"Trailblazer rank":"First Steps rank"}</span></div></article>
      </section>
      <section className="next-section"><div className="section-title"><span>次の目的地</span><h2>Next up in your journey</h2><button onClick={()=>setTab("catalog")}>View full catalog →</button></div><article className="feature-card"><img src="/assets/cards/01-panoramic-journey.png"/><div className="feature-shade"></div><div className="feature-copy"><span>{next.group} · {next.year}</span><h3>{next.season}</h3><p>{next.title}</p><button onClick={()=>toggle(next)}>{progress[next.id]?.watched?"✓ Logged":"Log as watched"}</button></div></article></section>
    </>}

    {tab === "catalog" && <section className="catalog paper"><div className="catalog-head"><div><span>全編目録</span><h2>The complete animated archive</h2><p>{filtered.length.toLocaleString()} entries across series, films, and special animation.</p></div><img src="/assets/core/03-travel-map-compass.png"/></div>
